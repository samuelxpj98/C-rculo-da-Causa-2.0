...
export const Mural: React.FC<MuralProps> = ({ actions, onAddClick, mission, isVisitor, currentUser }) => {
    ...
    return (
        <div className="relative w-full h-full bg-white">
            ...
            <div ref={containerRef} className="w-full h-full overflow-auto no-scrollbar relative cursor-grab bg-white">
                ...
                <div className="w-[4000px] h-[4000px] relative flex items-center justify-center transition-transform duration-200 origin-center ease-out" style={{ transform: `scale(${scale})` }}>
                        ...
                        {actions.map((action, index) => {
                            ...
                            return (
                                <div key={action.id} className="absolute transition-all duration-700 animate-in zoom-in fade-in" style={{ transform: `translate(${coord.x}px, ${coord.y}px)`, marginLeft: '-3rem', marginTop: '-3rem' }}>
                                    <div onClick={(e) => { e.stopPropagation(); setSelectedAction(action); }} className={`w-24 h-24 clip-hexagon flex flex-col items-center justify-center relative group hover:z-50 cursor-pointer p-1 transition-all duration-300 ${getActionColor(action.action)} hover:scale-110 shadow-sm`}>
                                        ...
                                        <span className="text-[9px] font-bold leading-none text-center uppercase drop-shadow-md z-10 line-clamp-1 w-full px-1 break-all tracking-tight text-white">{action.userName.split(' ')[0]}</span>
                                    </div>
                                </div>
                            );
                        })}
